...
\title{HPS SVT Operations Manual \\ v2.1}
\author{SVT On-Call Cell Phone: 757-310-7198 \\ 
Authors: \\
...
\date{March 22, 2016} % delete this line to display the current date
...
Motion and wire scanner & Matt Solt, Tim Nelson \\
SVT DAQ & Cameron Bravo, Omar Moreno \\
SVT EPICS controls & Omar Moreno, Matt Solt \\
MPOD power supply & Omar Moreno \\
PLC interlocks & Brian Eng \\
Cooling & Tim Nelson, Matt Solt \\
 \hline \hline
\end{tabular}
\end{center}
...
The SVT, shown in Figure~\ref{fig:SVT}, uses 7 layers of silicon extending from 5~cm to 90~cm downstream of the target inside of the PS vacuum chamber to measure charged particle trajectories.  
%=======================
\begin{figure}[htbp]
\begin{center}
    \includegraphics[width=7cm]{SVT}\includegraphics[width=7cm]{SVT_top}
\caption{The SVT shown inside of the pair spectrometer vacuum chamber.  The beam enters through the flange in the front of the vacuum box extension that houses services for the SVT.  These images show the detector before the addition of Layer 0 and upgrade of Layer 1.}
\label{fig:SVT}
\end{center}
\vspace*{-5mm}
\end{figure}
%=======================
To accommodate the passage of the beam, the SVT is built in two halves, top and bottom, so that each layer consists of a pair of modules, one above and one below the beam plane.  Each module uses silicon microstrip sensors placed back-to-back with a small stereo angle between sides to provide 3-d space points for the hits in a module.  Modules for Layers 0 and 1 have a single sensor on each side with readout at both ends, where the sense strips are split along the midline of the sensor into pairs of striplets. These new modules use slim-edge silicon which allows them to be placed closer to the beam than during previous runs. Modules for layers 2 and 3 have a single sensor on each side with readout at one end, while those for layers 4-6 are longer, with a pair of sensors on each side and readout at both ends.

Modules are supported in groups of three by a set of four support plates, as shown in Figure~\ref{fig:L1-3_bottom}. The top and bottom support plates for the back half of the SVT (layers 4-6) are stationary.  However, the supports for layers 0-3 can be opened and closed vertically around the beam, rotating around hinges behind layer 3 and moved by levers extending upstream to a pair of linear shifts outside of the magnet. The support plates are kinematically mounted inside a support box that installs into the pair spectrometer (PS) vacuum chamber, shown in Figure~\ref{fig:SVT_Box}.
%=======================
\begin{figure}[htpb]
\subfigure[\label{fig:L1-3_bottom}The lower support plate for Layers 0-3, showing the silicon (red) and readout electronics (green) of the modules, as well as the motion lever for opening and closing the SVT and the SVT beam scan wires.  This image shows the detector before the addition of Layer 0 and upgrade of Layer 1.]
{\includegraphics[width=6.5cm]{L1-3_bottom}}
\hspace{0.5cm}
\subfigure[\label{fig:SVT_Box}The SVT support box which contains the upper and lower support plates for Layers 0-3 and Layers 4-6, as well as the cooling plate housing the Front End Boards of the SVT DAQ.]
{\includegraphics[width=6.5cm]{SVT_Box}}
\caption{Key sub-assemblies of the SVT.}
\end{figure}
%=======================

The first stage of readout electronics is located on a hybrid circuit board at the end of each sensor.  Multiplexed analog signals from these boards are digitized by a set of 10 Front End Boards (FEBs) mounted to a separate cooling plate inside the SVT support box. Each FEB can control 4 hybrid/sensor units: a single module in layers 4-6 and two modules in layers 0-3. The FEBs also control the hybrids, provide regulated low-voltage power from a single input, and pass externally generated bias voltages (HV) through to the sensors.   The FEBs communicate with a set of 4 Signal Flange Boards (SFB), up to three per SFB, which transmit digital signals through the vacuum penetration.  The exterior side of each flange board converts digital to optical signals for communication with the RCE DAQ.  Power to the FEB are routed through a pair of Power Flange Boards, one for LV and one for HV, supplied by Wiener MPOD power supply modules in a crate on the pie-tower.

Cooling for the SVT is provided by a pair of chillers; one for the hybrids and sensors that operate at -20 $^\circ$C and one for the FEBs that operates at room temperature.  The FEBs have a single cooling loop, while the hybrids and sensors have two loops, one for the top and one for the bottom half of the SVT, where each of these loops runs first through the support structure for layers 0-3 and then through the structure for layers 4-6.  There are temperature sensors on every hybrid as well as sensors in the FEBs. The linear shifts, the cooling penetrations, and the signal/power penetrations are all located on a set of flanges on an extension vacuum box mounted to the upstream end of the PS vacuum chamber and which connects to the upstream beam line.
...
ISEG EHS F201p\_805F high precision high voltage modules provide the reverse bias voltage to the silicon sensors of the SVT. Each module has 16 output channels (floating). The three modules each supply bias to 12 sensors: SVT L0-3, L4-6 top or L4-6 bottom.  The bias supply to Layer 0 and Layer 1 are ganged, so that a single supply provides bias voltage to the sensors on both sides of the modules.  All other sensors in the detector have individual bias supply.
...
\hline
SVT modules & -5 $^\circ$C & 75 W & 10 W  \\
Piping to SVT modules & -10 $^\circ$C & 100 W & 0 \\
Set point, SVT chiller & -10 $^\circ$C & - & - \\
Front End Boards & 10 $^\circ$C & 100 W & 0  \\
Set point, FEB chiller & 4 $^\circ$C & - & - \\
 \hline \hline
\end{tabular}
...
\end{figure}
%=======================
Since the SVT is in vacuum, the cooling system is key for the safety and performance of the detector. The main heat sources are the sensor modules and their hybrid circuit boards and the Front End Boards (FEBs) which distribute power and DAQ to reduce the number of wires that must penetrate the vacuum flanges. An additional heat load results from the radiative heat exchange between the SVT U-support and the detector vacuum. The sensor modules need to be controlled to a sub-zero temperature, while the FEBs only need to be thermally managed to remove heat produced by their operation. While the FEBs have typically been operated close to room temperature, the degradation of some components on the FEBs requires them to be cooled to 10-15~$^\circ$C in order to maintain operability of some parts of the detector.  To maintain the ability to set the silicon and FEB temperatures separately, there are two separate chillers with independent loops and controls. The dew point in Hall B is typically  $\sim$15 $^\circ$C. The cold lines from the chillers to the vacuum box is insulated with pipe insulation.
...
\caption{Beam's eye view of wires and and L0 sensor edge}
\label{fig:beam}
\end{figure}
...
\item Turn on the SVT Sensor HV Power by pressing the \texttt{ALL SVT BIAS} \textbf{ON} button.
\item Beam interlock (under \texttt{ALL HV CONTROL}): Reset the interlock by pressing the \textbf{RESET} button, then the \texttt{180V} button. Wait for the status indicator to turn green.
\end{enumerate}
...
\item Beam interlock is enabled (green \texttt{Normal}, not red \texttt{Bypass}). If not enabled, enable in the SVT software interlock GUI.
...
If the beam is lost, the SVT beam interlock (if enabled) will lower the HV. In general, for beam trips:
\begin{enumerate}
