...
\title{HPS SVT Operations Manual \\ v1.2}
...
\date{February 30, 2016} % delete this line to display the current date
...
If this is the first run after setting up beam for HPS, the SVT will be open and the SVT Expert will need to be called in order to move the SVT into position. The experts-only procedure for establishing 0.5 mm running with the SVT may be found in Section~\ref{sec:proc_svt_0.5mm}.
...
If the beam is lost, the SVT software interlock lowers the HV to 5 V.  In general, for beam trips:
\begin{enumerate}
...
%\item If the beam interlock was enabled: Look at the SVT bias GUI, and verify that the \textbf{Measured Voltage} column reads as expected (default is 5V) for all channels. If not, call the expert. Continue with the procedure.
...
\item Reset the HV: Click the \textbf{RESET} button on the bias GUI to reset the interlock, then the \textbf{180V} button to reset the bias voltage.
\end{enumerate}
