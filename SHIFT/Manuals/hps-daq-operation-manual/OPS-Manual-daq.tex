...
\begin{enumerate}
\item A run database dialog box will appear. Fill in the information it asks for, and click OK.
...
\item A run database dialog box will appear, prefilled with the info you entered at Prestart. Select whether the run was good, change anything else you want to change, and click OK.
\item The \texttt{Prestart} button should appear.
\item In the \texttt{SVT summary GUI} under section \texttt{SVT DAQ STATUS} and check that both \texttt{DPM RUNSTATE}  and \texttt{DPM RUNSTATE}  are saying \texttt{Stopped}.
\end{enumerate}
\end{enumerate}
